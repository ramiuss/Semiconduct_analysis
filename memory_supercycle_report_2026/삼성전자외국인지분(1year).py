...
price_df.to_csv("data/yf_005930_1y.csv")  # CSV로 저장 (검증용)

price_raw = yf.download("000660.KS", start=start_date, end=end_date, progress=False)
if isinstance(price_raw.columns, pd.MultiIndex):
    price_raw.columns = price_raw.columns.get_level_values(0)
price_df = price_raw[["Open","High","Low","Close","Volume"]].copy()
price_df.index = pd.to_datetime(price_df.index).date
price_df.index.name = "Date"
price_df.to_csv("data/yf_000660_1y.csv")
print(len(price_df), "rows saved")
